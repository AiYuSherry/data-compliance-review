...
def process_desensitization(
    *,
    ...
    if is_text or suffix in TEXT_EXTENSIONS:
        ...
        sanitized, text_findings = engine.sanitize_text(raw, surface='text', locator='全文')
        ...
        output_file, doc_findings = process_docx(input_path, work_dir, engine, warnings)
        findings.extend(doc_findings)
        input_type = 'document'
    elif suffix in PDF_EXTENSIONS:
        output_file, pdf_findings = process_pdf(input_path, work_dir, engine, warnings)
        findings.extend(pdf_findings)
        input_type = 'pdf'
    elif suffix in TABLE_EXTENSIONS:
        output_file, table_findings = process_table(input_path, work_dir, engine, warnings)
        findings.extend(table_findings)
        input_type = 'table'
    elif suffix in JSON_EXTENSIONS:
        output_file, json_findings = process_json(input_path, work_dir, engine, warnings)
        findings.extend(json_findings)
        input_type = 'json'
    elif suffix in PRESENTATION_EXTENSIONS:
        output_file, presentation_findings = process_pptx(input_path, work_dir, engine, warnings)
        findings.extend(presentation_findings)
        input_type = 'presentation'
    elif suffix in IMAGE_EXTENSIONS:
        output_file, image_findings = process_image(input_path, work_dir, engine, warnings)
...
def process_docx(
    ...
    if input_path.suffix.lower() == '.doc':
        ...
        sanitized, findings = engine.sanitize_text(text, surface='doc', locator='全文')
    ...
    for index, paragraph in enumerate(document.paragraphs, start=1):
        if not paragraph.text:
            continue
        sanitized, findings = engine.sanitize_text(paragraph.text, surface='docx', locator=f'段落 {index}')
        all_findings.extend(findings)
        if findings:
            paragraph.text = sanitized
    ...
    for table_index, table in enumerate(document.tables, start=1):
        for row_index, row in enumerate(table.rows, start=1):
            for col_index, cell in enumerate(row.cells, start=1):
                ...
                sanitized, findings = engine.sanitize_text(cell.text, surface='docx_table', locator=locator)
                all_findings.extend(findings)
                if findings:
                    cell.text = sanitized
    ...
    return output_file, all_findings
...
def process_pdf(
    ...
    if fitz is not None:
        ...
        for page_index, page in enumerate(pdf, start=1):
            ...
            if page_text.strip():
                sanitized, findings = engine.sanitize_text(page_text, surface='pdf', locator=f'第 {page_index} 页')
                all_findings.extend(findings)
    ...
    for page_index, page in enumerate(pdf, start=1):
        ...
        redacted, page_findings = redact_image(page_image, work_dir / f'desensitized_output_page_{page_index}.png', engine, f'第 {page_index} 页')
        ...
        for sheet_name, frame in sheets.items():
            ...
            for row_index, row in sanitized_frame.iterrows():
                for column in sanitized_frame.columns:
                    ...
                    sanitized, findings = engine.sanitize_text(value, surface='xlsx_cell', locator=locator)
                    if findings:
                        sanitized_frame.at[row_index, column] = sanitized
                        all_findings.extend(findings)
    ...
    *,
    ...
    for row_index, row in enumerate(rows, start=1):
        for col_index, value in enumerate(row, start=1):
            sanitized, findings = engine.sanitize_text(value, surface='csv_cell', locator=f'R{row_index}C{col_index}')
            if findings:
                row[col_index - 1] = sanitized
                all_findings.extend(findings)
...
def process_json(input_path: Path, work_dir: Path, engine: Desensitizer, warnings: list[str]) -> tuple[Path, list[Finding]]:
    ...
    def walk(value: Any, path: str) -> Any:
        if isinstance(value, str):
            sanitized, findings = engine.sanitize_text(value, surface='json_value', locator=path)
            all_findings.extend(findings)
            return sanitized
...
def process_json_lines(
    ...
    def walk(value: Any, path: str) -> Any:
        if isinstance(value, str):
            sanitized, findings = engine.sanitize_text(value, surface='jsonl_value', locator=path)
            all_findings.extend(findings)
            return sanitized
        if isinstance(value, list):
            return [walk(item, f'{path}[{index}]') for index, item in enumerate(value)]
    ...
    for line_index, line in enumerate(input_path.read_text(encoding='utf-8').splitlines(), start=1):
        ...
        try:
            ...
            sanitized, findings = engine.sanitize_text(line, surface='jsonl_line', locator=f'行 {line_index}')
            all_findings.extend(findings)
    ...
    for slide_index, slide in enumerate(presentation.slides, start=1):
        for shape_index, shape in enumerate(slide.shapes, start=1):
            if getattr(shape, 'has_text_frame', False) and shape.text:
                ...
                sanitized, findings = engine.sanitize_text(shape.text, surface='pptx_text', locator=locator)
                if findings:
            ...
            if getattr(shape, 'has_table', False):
                for row_index, row in enumerate(shape.table.rows, start=1):
                    for col_index, cell in enumerate(row.cells, start=1):
                        ...
                        sanitized, findings = engine.sanitize_text(cell.text, surface='pptx_table', locator=locator)
                        if findings:
                            cell.text = sanitized
                            all_findings.extend(findings)
...
def redact_image(input_path: Path, output_file: Path, engine: Desensitizer, locator_prefix: str) -> tuple[Path, list[Finding]]:
    ...
    for index, token in enumerate(data.get('text', [])):
        ...
        if not text:
            continue
        sanitized, findings = engine.sanitize_text(text, surface='image_text', locator=f'{locator_prefix} OCR#{index + 1}')
        if not findings or sanitized == text:
            continue
